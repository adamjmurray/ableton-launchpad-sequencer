...
class Pattern {
  static initClass() {
    ...
    this.GATE_DURATIONS = [null, 1,  2,   4,   8  ];
    this.OCTAVES =        [null, 12, 24, -12, -24 ];
    this.MODS =           [null, 0,  42,  85,  127];
    this.randomVelocity = this.randomPitch;
    ...
    this.processors = {
      ...
      'scale gate':    (note, value) => { note.duration = 1; note.pitch = this.SCALE.map(note.pitch, value-1); },
      'velocity gate': (note, value) => { note.duration = 1; note.velocity += ((127 - note.velocity) * (value-1)) / 3; },
      'duration gate': (note, value) => { note.duration = this.GATE_DURATIONS[value]; },
  
      'pitch +':    (note, value) => { note.pitch     += value; },
      'pitch -':    (note, value) => { note.pitch     -= value; },
      'scale +':    (note, value) => { note.pitch      = this.SCALE.map(note.pitch, value); },
      'scale -':    (note, value) => { note.pitch      = this.SCALE.map(note.pitch, -value); },
      'octave':     (note, value) => { note.pitch     += this.OCTAVES[value]; },
      ...
      'modulation': (note, value) => { note.modulation = this.MODS[value]; },
      'aftertouch': (note, value) => { note.aftertouch = this.MODS[value]; },
      ...
      'chaos'      :(note, value) => {
        switch (value) {
          case 1: note.pitch    = this.randomPitch(); break;
          case 2: note.velocity = this.randomVelocity(); break;
          case 3: note.duration = this.randomDuration(); break;
          case 4: [note.pitch, note.velocity, note.duration] = Array.from([this.randomPitch(), this.randomVelocity(), this.randomDuration()]); break;
        }
      }
    };
  }
  ...
  random() {
    for (let i = this.start, { end } = this; i <= end; i++) {
      this.sequence[i] = Math.floor(5*Math.random());
    }
  }
  ...
  setType(type) {
    this.type = type;
    this._process = Pattern.processors[type] || NOOP;
  }
  ...
  processNote(note, clock) {
    ...
    if (value > 0) { this._process(note, value); } // NOTE, major assumption: 0 is always a NOOP! But a good optimization...
     // MIDI CC modulation values for each step value
  }

  static randomPitch() { return Math.floor(Math.random() * 128); }
  static randomDuration() { return Math.random() * 8; }

...
class LaunchpadController {
  static initClass() {
    ...
    // Pattern "Ops" modes
    this.LENGTH_MODE = 0; // pattern start/end (length)
    this.STEPS_MODE = 1;
      // pattern step values
  }
  ...
  ctlin(cc, value) {
    ...
    if (value > 0) {
      ...
      if (this.heldTop == null) { this.heldTop = index; }
    } else {
      // @_onTopUp(index)
      if (this.heldTop === index) { this.heldTop = null; }
    }
  }
  ...
  notein(pitch, velocity) {
    ...
    if (x > 7) {
      if (velocity > 0) { this._onRightDown(y); } // else @_onRightUp(y)
    } else {
      if (velocity > 0) {
